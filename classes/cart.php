	...
	{	
		...
		public function add_to_cart($quantity,$id)
		{
			...
			$query_insert ="INSERT INTO tbl_cart(productId,quantity,sId,image,price,productName) VALUES('$id','$quantity','$sId','$image','$price','$productName')";
				$insert_cart=$this->db->insert($query_insert);//chon va co quyen thuc thi de chon ra
				if ($insert_cart) {
		...
		public function insertOrder($customer_id){//order mua hang
			$sId=session_id();
			$query="SELECT * FROM tbl_cart WHERE sId='$sId'";
		...
		public function insertHoaDon($customer_id){//order mua hang
			$sId=session_id();
			$query="SELECT * FROM tbl_cart WHERE sId='$sId'";
			...
			if ($get_product) {
				...
				while ($result = $get_product->fetch_assoc()) {
					...
					$image=$result['image'];
					$customer_id=$customer_id;
					$query_order ="INSERT INTO tbl_hoadon(productId,productName,quantity,price,image,customer_id) VALUES('$productId','$productName','$quantity','$price','$image','$customer_id')";
					$insert_order=$this->db->insert($query_order);
				}
			}
		}
		public function getAMPrice($customer_id){//giá success
			$query="SELECT * FROM tbl_order WHERE customer_id='$customer_id'";
			$get_price=$this->db->select($query);
			return $get_price;
		}
		public function get_cart_ordered($customer_id){//trang hóa đơn đã được order
			$query="SELECT * FROM tbl_order WHERE customer_id='$customer_id'";
			$get_price=$this->db->select($query);
			return $get_price;
		...
		public function get_inbox_cartHD(){
			$query="SELECT * FROM tbl_hoadon ORDER BY date_order";
			$get_inbox_cart=$this->db->select($query);
			return $get_inbox_cart;
		}
		//inbox
